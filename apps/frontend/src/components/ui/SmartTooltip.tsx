...
export const SmartTooltip: React.FC<SmartTooltipProps> = ({
  ...
  const updatePosition = useCallback(() => {
     ...
     // Flip Vertical
     if (top < padding && placement === 'top') top = triggerRect.bottom + 8;
     if (top + tooltipRect.height > winH - padding && placement === 'bottom') top = triggerRect.top - tooltipRect.height - 8;
    ...
    <>
      ...
      {isVisible && createPortal(
        <div
            ...
            className={`fixed z-[9999] animate-[fadeIn_0.15s_ease-out] w-max max-w-[90vw] xs:max-w-[280px] sm:max-w-[300px] md:max-w-[320px] ${className}`}
            onMouseEnter={triggerMode === 'hover' ? show : undefined}
            onMouseLeave={triggerMode === 'hover' ? hide : undefined}
        >
           <div className="bg-slate-950/95 backdrop-blur-xl border border-cyan-500/40 rounded-lg sm:rounded-xl shadow-[0_8px_32px_rgba(0,0,0,0.8)] p-2.5 sm:p-3 text-left relative overflow-hidden ring-1 ring-cyan-500/20">
              ...
              <div className="relative z-10 max-h-[50vh] sm:max-h-[60vh] overflow-y-auto custom-scrollbar overscroll-contain">{content}</div>
           </div>
        </div>, document.body
      )}
    </>
