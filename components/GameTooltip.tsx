    ...
    return (
        <div className="flex flex-col gap-3 min-w-[220px]">
            ...
            {stats && stats.length > 0 && (
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-2.5 bg-black/30 p-2.5 rounded-lg border border-white/5">
                    {stats.map((s, i) => (
                        <div key={i} className="flex flex-col">
                            <span className="text-slate-500 uppercase text-[9px] font-bold tracking-wider">{s.label}</span>
                            <span className={`font-mono text-xs font-bold ${s.color || 'text-slate-200'} mt-0.5`}>{s.value}</span>
                        </div>
                    ))}
                </div>
            )}
