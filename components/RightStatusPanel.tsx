...
export const RightStatusPanel: React.FC<RightStatusPanelProps> = ({ isOpen = false, onClose }) => {
    ...
    // --- LOGIC: Drawer Classes ---
    // Mobile: Fixed drawer sliding from right (90% width or max 384px)
    // Desktop: Static sidebar taking full height of parent
    const drawerClasses = `
    ...
    return (
        <>
            ...
            <aside className={drawerClasses}>
                {/* Main Flex Container: Ensures header/footer stay fixed while content scrolls */}
                <div className="flex flex-col h-full w-full relative overflow-hidden">
                    ...
                    {/* 1. HEADER (Mobile Only) - Fixed Top */}
                    ...
                    {/* 2. THREAT MONITOR - Fixed Top (Always Visible) */}
                    <div className={`p-3 md:p-4 border-b border-white/10 shrink-0 relative z-10 transition-colors ${activeWar ? 'bg-red-950/30' : 'bg-black/10'}`}>
                        ...
                        {/* Attacks List (Scrollable if too many, but contained in fixed height area) */}
                                        ...
                                        <div key={attack.id} className="bg-red-950/40 border border-red-500/30 p-1.5 rounded text-xs relative overflow-hidden group">
                                            ...
                                            {attack.isScouted && (
                                                <div className="pl-2 text-[9px] text-red-400/80 font-mono">
                                                    Intel: {Object.values(attack.units).reduce((a:any,b:any)=>a+b,0)} units
                                                </div>
                                            )}
                                        </div>
                                    ))}
                    ...
                    {/* 3. SCROLLABLE CONTENT (Queues & Missions) - Takes remaining height */}
                    <div className="flex-1 overflow-y-auto min-h-0 custom-scrollbar p-3 md:p-4 space-y-5 relative z-10">
                        ...
                        {/* Spacer for bottom navigation / safe area on mobile */}
