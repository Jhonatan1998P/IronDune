        ...
        <>
            ...
            <nav className="w-full h-full glass-panel border-t border-white/10 bg-slate-950/95 backdrop-blur-xl flex justify-around items-center px-1 pb-safe shadow-[0_-5px_30px_rgba(0,0,0,0.5)] relative z-50">
                {primaryItems.map(item => {
                    ...
                    return (
                        <button
                            ...
                            onClick={() => handleTabSelect(item.id)}
                            className={`flex flex-col items-center justify-center p-1 rounded-lg transition-all duration-200 relative group flex-1 h-full ${isActive ? 'text-cyan-400' : 'text-slate-500 hover:text-slate-300'}`}
                        >
                            {isActive && (
                                <div className="absolute top-0 w-8 h-0.5 bg-cyan-500 shadow-[0_0_10px_#06b6d4]"></div>
                            )}
                            
                            <div className={`transition-transform duration-300 ${isActive ? '-translate-y-1' : ''} mb-0.5`}>
                                <item.icon />
                            </div>
                ...
                <button
                    ...
                    className={`flex flex-col items-center justify-center p-1 rounded-lg transition-all duration-200 relative group flex-1 h-full ${isMenuOpen ? 'text-white' : 'text-slate-500'}`}
                >
                    <div className={`transition-transform duration-300 ${isMenuOpen ? 'rotate-90 scale-110' : ''} mb-0.5`}>
                        <Icons.Menu />
                    </div>
                    <span className={`text-[9px] font-bold uppercase tracking-wider ${isMenuOpen ? 'opacity-100' : 'opacity-60'}`}>
