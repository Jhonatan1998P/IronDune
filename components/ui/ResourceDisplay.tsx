...
export const ResourceDisplay: React.FC<{ label: string; value: number; max: number; color: string; production?: number; upkeep?: number; icon?: React.ReactNode }> = ({ label, value, max, color, production = 0, upkeep = 0, icon }) => {
  const { t } = useLanguage();
    ...
    <SmartTooltip 
        ...
        <div className="space-y-2 min-w-[180px]">
            ...
            <div className="font-mono text-[10px] space-y-1">
                <div className="flex justify-between"><span className="text-slate-400">{t.common.ui.inventory}:</span><span className="text-white">{Math.floor(value).toLocaleString()} / {Math.floor(max).toLocaleString()}</span></div>
                <div className="flex justify-between text-emerald-400"><span>{t.common.ui.income}:</span><span>+{fmt(production)}/10m</span></div>
                <div className="flex justify-between text-red-400"><span>{t.common.stats.upkeep}:</span><span>-{fmt(upkeep)}/10m</span></div>
                <div className="border-t border-white/5 mt-1 pt-1 flex justify-between font-bold">
                    ...
                    <span className={net >= 0 ? "text-emerald-400" : "text-red-500"}>{net > 0 ? '+' : ''}{fmt(net)}/10m</span>
                </div>
            </div>
      ...
      <div className={`flex flex-col items-center justify-center bg-black/40 px-2 md:px-3 py-1.5 rounded min-w-[60px] lg:min-w-0 lg:w-full border border-white/5 hover:bg-white/5 transition-all relative overflow-hidden h-[44px] lg:h-[50px] group shrink-0`}>
        ...
        <div className="hidden lg:flex w-full justify-between items-center gap-2 xl:gap-4 px-1">
            <div className="flex flex-col items-start min-w-0">
                ...
                <span className={`text-sm font-mono font-bold text-white leading-none mt-0.5 truncate`}>{Math.floor(value).toLocaleString()}</span>
            </div>
