...
interface TutorialOverlayProps {
    gameState: any; // Kept for prop compat
    activeTab: TabType;
}
...
export const TutorialOverlay: React.FC<TutorialOverlayProps> = ({ activeTab }) => {
    ...
    const getTargetElement = () => {
        ...
        // 1. Navigation Logic: If we are not on the correct tab, point to the Nav Button
        if (currentStep.targetTab !== activeTab) {
            const desktopTab = document.getElementById(`tab-${currentStep.targetTab}`);
            // Check visibility of desktop tab (it might be hidden on mobile)
            ...
            // If checking secondary menu on mobile
            if (fab) {
                 const fabStyle = window.getComputedStyle(fab);
                 const fabOpacity = parseFloat(fabStyle.opacity);
                 if (fabOpacity > 0.5) return fab;
            }
            // Fallback to mobile tab bar
            return mobileTab;
        } 
        
        // 2. Element Logic: If we are on correct tab, point to specific element
    ...
    const shouldShow = () => {
        ...
        // If step has a specific progress condition (like "Wait 5 seconds"), hide pointer if done but not claimed
        // However, usually we want to point until claimed.
        // if (currentStep.progressCondition && currentStep.progressCondition(gameState)) return false; 
        return true;
    };
    ...
    useEffect(() => {
        ...
        if (element) {
            // Only scroll if we haven't scrolled recently to avoid fighting the user
    ...
    useEffect(() => {
        ...
        const updateRect = () => {
             ...
             if (element) {
                 ...
                 // Smooth update or instant? React state updates are batched, 
                 // but for a tutorial overlay following scrolling, we want responsiveness.
                 setTargetRect(prev => {
                     if (!prev || 
    ...
    // We want the arrow to point TOWARDS the target from the most spacious side.
    ...
    if (spaceBottom > 150) {
        // Arrow BELOW, pointing UP
        arrowRot = 180;
        arrowTop = targetRect.bottom + 10;
        arrowLeft = targetRect.left + (targetRect.width / 2) - 20; // Center arrow (40px width)
        textPosClass = "top-4"; 
    ...
    return (
        <div className="fixed inset-0 z-[9998] pointer-events-none overflow-hidden transition-opacity duration-300 ease-in-out">
            ...
            {/* 1. SPOTLIGHT EFFECT (The "Hole") */}
            {/* Uses a massive box-shadow to darken everything ELSE */}
            <div 
            ...
            {/* 3. OPTIONAL: Text Label near Arrow (for context if needed) */}
            <div 
