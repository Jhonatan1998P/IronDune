...
export const PvpAttackModal: React.FC<PvpAttackModalProps> = ({ target, gameState, onClose, onAttackSent }) => {
    ...
    return (
        <div className="absolute inset-0 z-[100] flex items-center justify-center bg-black/80 backdrop-blur-md animate-[fadeIn_0.2s_ease-out] p-3 md:p-6">
            <div 
                className={`
                    w-full max-w-lg glass-panel rounded-2xl border flex flex-col
                    ${isWarTarget ? 'border-red-500/30' : 'border-white/10'}
                    h-[calc(100vh-140px)] md:h-[calc(90vh-60px)]
                `}
            >
                {/* Header */}
                <div className={`shrink-0 p-4 border-b border-white/10 flex items-center justify-between ${isWarTarget ? 'bg-red-950/30' : 'bg-black/40'}`}>
                    <div className="flex items-center gap-3">
                        <div className={`p-2.5 rounded-xl ${isWarTarget ? 'bg-red-500/20 text-red-400' : 'bg-yellow-500/20 text-yellow-400'}`}>
                            <Icons.Army className="w-5 h-5" />
                        </div>
                        <div>
                            <span className={`text-[9px] uppercase tracking-widest font-bold ${isWarTarget ? 'text-red-400' : 'text-yellow-400'}`}>
                                {isWarTarget ? t.common.war.title : t.common.ui.tactical_op}
                            </span>
                            <h2 className="font-tech text-base text-white uppercase tracking-wide truncate max-w-[180px] md:max-w-[250px]">
                                {target.name}
                            </h2>
                        </div>
                    </div>
                    <button onClick={onClose} className="p-2 rounded-lg bg-black/40 hover:bg-white/10 text-slate-500 hover:text-white transition-colors">
                ...
                <div className="flex-1 overflow-y-auto custom-scrollbar p-4 space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        <div className="glass-panel p-3 rounded-xl border border-white/5 flex flex-col items-center justify-center">
                            <div className="text-[9px] text-slate-500 uppercase tracking-widest font-bold mb-1">{t.common.ui.est_power}</div>
                            <div className="font-mono text-lg text-yellow-400 font-bold">{formatNumber(target.score)}</div>
                        </div>
                        <div className="glass-panel p-3 rounded-xl border border-white/5 flex flex-col items-center justify-center">
                            <div className="text-[9px] text-slate-500 uppercase tracking-widest font-bold mb-1">{t.common.ui.travel_time}</div>
                            <div className={`font-mono text-lg font-bold ${useDiamond ? 'text-cyan-400' : 'text-white'}`}>
                    ...
                    <div 
                        ...
                        className={`flex items-center justify-between p-3 rounded-xl border cursor-pointer transition-all ${useDiamond ? 'bg-cyan-950/30 border-cyan-500/30' : 'bg-black/40 border-white/5 hover:border-white/20'} ${!hasDiamonds ? 'opacity-50 cursor-not-allowed' : ''}`}
                    >
                        <div className="flex items-center gap-3">
                            <div className={`w-5 h-5 rounded border-2 flex items-center justify-center transition-colors ${useDiamond ? 'bg-cyan-500 border-cyan-500' : 'border-slate-600'}`}>
                                {useDiamond && <Icons.Check className="w-3 h-3 text-black" />}
                            </div>
                            <div>
                                <div className="text-xs font-bold text-cyan-100 uppercase tracking-wider">{t.common.actions.rapid_deployment}</div>
                                <div className="text-[9px] text-slate-500">{t.common.ui.instant_arrival_desc || "Reduce travel time by 80%"}</div>
                            </div>
                        </div>
                        <div className="flex items-center gap-1 bg-cyan-950/50 px-2 py-1 rounded-lg border border-cyan-500/30">
                            <Icons.Resources.Diamond className="w-3.5 h-3.5 text-cyan-400" />
                    ...
                    <div className="space-y-3">
                        ...
                        <div className="space-y-2">
                            {availableUnits.length === 0 ? (
                                <div className="text-center text-slate-500 text-xs py-8 bg-black/20 rounded-xl border border-dashed border-white/10">
                                ...
                                availableUnits.map(([uTypeString, max]) => {
                                    ...
                                    return (
                                        <div key={uType} className="flex items-center justify-between p-2.5 bg-black/20 hover:bg-white/[0.03] border border-white/5 rounded-xl transition-all">
                                            ...
                                            <div className="flex items-center gap-0.5 bg-black/40 p-1 rounded-lg border border-white/5">
                                                <button 
                                                    ...
                                                    className="w-8 h-8 bg-white/5 hover:bg-red-500/20 rounded-md text-slate-400 hover:text-red-400 flex items-center justify-center transition-colors text-sm"
                                                >
                                                    −
                                                </button>
                                                ...
                                                <input 
                                                    ...
                                                    placeholder="0"
                                                    className="w-12 bg-transparent text-center font-mono text-xs font-bold text-yellow-400 focus:outline-none"
                                                />

                                                <button 
                                                    onClick={() => handleUnitChange(uType, 1)} 
                                                    className="w-8 h-8 bg-white/5 hover:bg-emerald-500/20 rounded-md text-slate-400 hover:text-emerald-400 flex items-center justify-center transition-colors text-sm"
                                                >
                                                    +
                                                </button>
                                            </div>
                ...
                <div className="shrink-0 p-4 border-t border-white/10 bg-black/40 space-y-3">
