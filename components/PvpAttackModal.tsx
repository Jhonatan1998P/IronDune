...
export const PvpAttackModal: React.FC<PvpAttackModalProps> = ({ target, gameState, onClose, onAttackSent }) => {
    ...
    return (
        <div className="fixed inset-0 z-[100] flex items-end md:items-center justify-center bg-black/80 backdrop-blur-md animate-[fadeIn_0.2s_ease-out]">
            <div
                className={`
                    w-full glass-panel rounded-t-2xl md:rounded-2xl border flex flex-col
                    ${isWarTarget ? 'border-red-500/30' : 'border-white/10'}
                    h-[85dvh] md:h-auto md:max-h-[85dvh] max-w-sm sm:max-w-md md:max-w-lg
                    mx-0 sm:mx-3 md:mx-4
                `}
            >
                {/* Header */}
                <div className={`shrink-0 p-3 sm:p-4 border-b border-white/10 flex items-center justify-between ${isWarTarget ? 'bg-red-950/30' : 'bg-black/40'}`}>
                    <div className="flex items-center gap-2 sm:gap-3 min-w-0 flex-1">
                        <div className={`p-2 rounded-lg sm:rounded-xl shrink-0 ${isWarTarget ? 'bg-red-500/20 text-red-400' : 'bg-yellow-500/20 text-yellow-400'}`}>
                            <Icons.Army className="w-4 h-4 sm:w-5 sm:h-5" />
                        </div>
                        <div className="min-w-0">
                            <span className={`text-[8px] sm:text-[9px] md:text-[10px] uppercase tracking-widest font-bold ${isWarTarget ? 'text-red-400' : 'text-yellow-400'}`}>
                    ...
                    <div className="p-3 sm:p-4 space-y-3 sm:space-y-4">
                        ...
                        <div className="grid grid-cols-2 gap-2 sm:gap-3">
                            <div className="glass-panel p-2 sm:p-3 rounded-lg sm:rounded-xl border border-white/5 flex flex-col items-center justify-center">
                                ...
                                <div className="font-mono text-sm sm:text-lg md:text-xl text-yellow-400 font-bold">{formatNumber(target.score)}</div>
                            </div>
                            <div className="glass-panel p-2 sm:p-3 rounded-lg sm:rounded-xl border border-white/5 flex flex-col items-center justify-center">
                                ...
                                <div className={`font-mono text-sm sm:text-lg md:text-xl font-bold ${useDiamond ? 'text-cyan-400' : 'text-white'}`}>
                        ...
                        <div
                            ...
                            className={`flex items-center justify-between p-2 sm:p-3 rounded-lg sm:rounded-xl border cursor-pointer transition-all ${useDiamond ? 'bg-cyan-950/30 border-cyan-500/30' : 'bg-black/40 border-white/5 hover:border-white/20'} ${!hasDiamonds ? 'opacity-50 cursor-not-allowed' : ''}`}
                        >
                            <div className="flex items-center gap-2 sm:gap-3">
                                <div className={`w-4 h-4 sm:w-5 sm:h-5 rounded border-2 flex items-center justify-center transition-colors shrink-0 ${useDiamond ? 'bg-cyan-500 border-cyan-500' : 'border-slate-600'}`}>
                                    {useDiamond && <Icons.Check className="w-2.5 h-2.5 sm:w-3 sm:h-3 text-black" />}
                                </div>
                                <div>
                                    <div className="text-[10px] sm:text-xs font-bold text-cyan-100 uppercase tracking-wider">{t.common.actions.rapid_deployment}</div>
                                    <div className="text-[9px] sm:text-[10px] text-slate-500 hidden sm:block">{t.common.ui.instant_arrival_desc || "Reduce travel time by 80%"}</div>
                                </div>
                            </div>
                                            ...
                                            <div key={uType} className="flex items-center justify-between p-2 sm:p-2.5 bg-black/20 hover:bg-white/[0.03] border border-white/5 rounded-lg sm:rounded-xl transition-all">
                                                ...
                                                <div className="flex items-center gap-0.5 bg-black/40 p-0.5 sm:p-1 rounded-md sm:rounded-lg border border-white/5">
                                                    ...
                                                    <input
                                                        ...
                                                        placeholder="0"
                                                        className="w-9 sm:w-12 bg-transparent text-center font-mono text-xs sm:text-sm font-bold text-yellow-400 focus:outline-none [-moz-appearance:textfield] [&::-webkit-outer-spin-button]:m-0 [&::-webkit-inner-spin-button]:m-0"
                                                    />
                ...
                <div className="shrink-0 p-3 sm:p-4 border-t border-white/10 bg-black/40 space-y-2 sm:space-y-3">
                    <div className="flex justify-between items-center">
                        <span className="text-[9px] sm:text-[10px] text-slate-500 uppercase tracking-widest font-bold">{t.common.ui.total_troops}</span>
                    ...
                    {!isWarTarget ? (
                        <button
                            ...
                            className={`
                                w-full py-2.5 sm:py-3 rounded-xl text-xs font-bold uppercase tracking-widest transition-all active:scale-[0.98]
