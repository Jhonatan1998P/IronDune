...
import { GameState, LogEntry, MissionDuration, ResourceType, UnitType } from '../../../types';
...
export const executeCampaignAttack = (state: GameState, levelId: number, playerUnits: Partial<Record<UnitType, number>>): ActionResult => {
    const now = Date.now();
    if (now < state.lastCampaignMissionFinishedTime + CAMPAIGN_COOLDOWN) return { success: false, errorKey: 'campaign_cooldown' };
    if (state.activeMissions.some(m => m.type === 'CAMPAIGN_ATTACK')) return { success: false, errorKey: 'campaign_busy' };
    ...
    const endTime = now + CAMPAIGN_TRAVEL_TIME;
    ...
    const newState = {
        ...state,
        units: newUnits,
        activeMissions: [ ...state.activeMissions, { id: missionId, type: 'CAMPAIGN_ATTACK' as const, startTime: now, endTime, duration: 7.5, units: playerUnits, levelId } ]
    };
    return { success: true, newState };
};
