...
export const useGameActions = (
  ...
  const executeDiamondExchange = useCallback((targetResource: ResourceType, amount: number) => {
      setGameState(prev => {
          ...
          if (result.success && result.newState) {
              if (result.log) addLog(result.log.messageKey, result.log.type, result.log.params);
              return result.newState;
          }
          if (result.errorKey) addLog(result.errorKey, 'info');
          return prev;
  ...
  return {
    ...
    acceptTutorialStep, claimTutorialReward, toggleTutorialMinimize, spyOnAttacker
  };
};
